...
while line = gets
  ...
  when "tools/list" then send_response(id, { tools: TOOLS })
  ...
  when "tools/call"
    ...
    case tool_name
    when "echo" then send_response(id, { content: [{ type: "text", text: args["message"] }] })
    ...
    when "fail" then send_response(id, { content: [{ type: "text", text: "error" }], isError: true })
